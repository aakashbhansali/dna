...
=======
        ...
        if (count($first_sample->i5_index)>0) {
>>>>>>> 74f4f3ed11e075bf472395ccf9ba5e4da8c1c03f
...
>>>>>>> 74f4f3ed11e075bf472395ccf9ba5e4da8c1c03f
    ...
    public function getCSVHeader($run, $number)
    {
        $tmpString = "";
        if ($number == 1) {
            $tmpString = "[Header]" . $this->addExtraCommas(10);
            return $tmpString;
        } else if ($number == 2) {
            $iem_file_version = DB::table('iem_file_version')->where('default', $run->iem_file_version_id)->first();
            $tmpString = "IEMFileVersion," . $iem_file_version->file_version . $this->addExtraCommas(9);
            return $tmpString;
        } else if ($number == 3) {
            $user = Auth::user();
            $tmpString = "InvestigatorName," . $user->name . $this->addExtraCommas(9);
            return $tmpString;
        } else if ($number == 4) {
            $projectId = DB::table('project_group')->where('id', $run->project_group_id)->first();
            $tmpString = "ProjectName," . $projectId->name . $this->addExtraCommas(9);
            return $tmpString;
        } else if ($number == 5) {
            $tmpString = "ExperimentName," . $run->experiment_name . $this->addExtraCommas(9);
            return $tmpString;
        } else if ($number == 6) {
            $tmpString = "Date," . $run->run_date->format('d-m-Y') . $this->addExtraCommas(9);
            return $tmpString;
        } else if ($number == 7) {
            $workFlow = DB::table('work_flow')->where('id', $run->work_flow_id)->first();
            $tmpString = "Workflow," . $workFlow->value . $this->addExtraCommas(9);
            return $tmpString;
        } else if ($number == 8) {
            $application = DB::table('application')->where('id', $run->application_id)->first();
            $tmpString = "Application," . $application->application . $this->addExtraCommas(9);
            return $tmpString;
        } else if ($number == 9) {
            $assay = DB::table('assay')->where('id', $run->assay_id)->first();
            $tmpString = "Assay," . $assay->name . $this->addExtraCommas(9);
            return $tmpString;
        } else if ($number == 10) {
            $tmpString = "Description," . $run->description . $this->addExtraCommas(9);
            return $tmpString;
        } else if ($number == 11) {
            $chemistry = DB::table('chemistry')->where('id', $run->chemistry_id)->first();
            $tmpString = "Chemistry," . $chemistry->chemistry . $this->addExtraCommas(9);
            return $tmpString;
        } else if ($number == 12) {
            $tmpString = $this->addExtraCommas(10);
            return $tmpString;
        } else if ($number == 13) {
            $tmpString = "[Reads]" . $this->addExtraCommas(10);
            return $tmpString;
        } else if ($number == 14) {
            $tmpString = $run->read1 . $this->addExtraCommas(10);
            return $tmpString;
        } else if ($number == 15) {
            $tmpString = $run->read2 . "," . $this->addExtraCommas(9);
            return $tmpString;
        } else if ($number == 16) {
            $tmpString = $this->addExtraCommas(10);
            return $tmpString;
        } else if ($number == 17) {
            $tmpString = "[Settings]" . $this->addExtraCommas(10);
            return $tmpString;
        } else if ($number == 18) {
            $adapter = DB::table('adaptor')->where('id', $run->iem_file_version_id)->first();
            $tmpString = "Adapter," . $adapter->value . $this->addExtraCommas(9);
            return $tmpString;
        } else if ($number == 19) {
            $tmpString = $this->addExtraCommas(10);
            return $tmpString;
        } else if ($number == 20) {
            $tmpString = "[Data]" . $this->addExtraCommas(10);
            return $tmpString;
        } else {
            return $tmpString;
        }
